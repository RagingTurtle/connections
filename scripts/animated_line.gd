...
func clear_and_start(start_pos: Vector2) -> void:
	clear_points()
	add_point(to_local(start_pos))
	
func draw_to_target(global_target_pos: Vector2, successfully_connected: bool) -> void:
	if active_tween and active_tween.is_running():
		active_tween.kill()
	
	var local_target: Vector2 = to_local(global_target_pos)
	...
	var new_idx: int = get_point_count() - 1
	
	active_tween = create_tween()
	
	active_tween.tween_method(
		_update_point_position.bind(new_idx),
		start_local_pos,
		local_target,
		duration
	)
	...
	if not successfully_connected:
		...
		active_tween.tween_callback(remove_point.bind(new_idx))
