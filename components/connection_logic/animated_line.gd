...
func _ready() -> void:
	var manager := get_parent() as Node2D
	if manager:
...
func draw_to_target(target_global_pos: Vector2, is_valid: bool) -> void:
	...
	var start_local_pos := get_point_position(last_idx)
	var distance := start_local_pos.distance_to(local_target)
	
	if distance < 0.01:
		return
		
	var duration := distance / LINE_SPEED
	active_tween = create_tween()
	
	var point_index := get_point_count()
